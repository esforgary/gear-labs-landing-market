...
const Reviews: React.FC = () => {
  ...
  const tempComments: ReviewItem[] = [
    { id: 1, name: "Alice", date: formatDate(new Date()), rating: 4, text: "Очень интересный продукт, понравилось всё, особенно внимание к деталям. Очень интересный продукт, понравилось всё, особенно внимание к деталям.", avatarColor: avatarColors[0] },
    { id: 2, name: "Bob", date: formatDate(new Date()), rating: 5, text: "Отличная поддержка клиентов, помогли с настройкой быстро и удобно.", avatarColor: avatarColors[1] },
    { id: 3, name: "Charlie", date: formatDate(new Date()), rating: 3, text: "В целом хороший сервис, но есть моменты, которые можно улучшить.", avatarColor: avatarColors[2] },
    { id: 4, name: "Diana", date: formatDate(new Date()), rating: 4, text: "Продукт полностью соответствует описанию, буду рекомендовать друзьям.", avatarColor: avatarColors[3] },
    { id: 5, name: "Edward", date: formatDate(new Date()), rating: 5, text: "Очень доволен, качественно, надежно и удобно.", avatarColor: avatarColors[4] },
  ];
  ...
  // Инициализация первых трёх
  useEffect(() => {
    setReviews(tempComments.slice(0, 3));
  }, []);
    ...
    const interval = setInterval(() => {
      ...
      setTimeout(() => {
        setReviews((prev) => {
          ...
          const currentIndexInFull = tempComments.findIndex(r => r.id === currentReview.id);
          const nextIndex = (currentIndexInFull + 3) % tempComments.length; // ← ТВОЙ +3
          newReviews[positionToChange] = tempComments[nextIndex];
          return newReviews;
        });
      ...
      <div className="reviews-grid">
        <div className="column-left scroll-animate">
          <div className="main-review review-card">
            ...
            <div className={`send-arrow ${mainText.trim() ? "active" : ""}`} onClick={handleSend}>
