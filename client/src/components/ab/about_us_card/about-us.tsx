...
type CardKey = "fast" | "reliable" | "stylish" | "practical" | "caring";
...
const cardShells: Array<Pick<CardData, "key" | "svg">> = [
  {
    key: "fast",
    svg: (
...
const AboutUsCard: React.FC = () => {
  ...
  const cards = useMemo<CardData[]>(
    () =>
      cardShells.map((card) => ({
        ...card,
        title: t(`about.card.${card.key}.title`),
        description: t(`about.card.${card.key}.text`),
      })),
    [t]
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveIndex(prev => (prev + 1) % cardShells.length);
    }, 15000);
  ...
  return (
    <div className="about-us-container">
      <div className="pagination-about-us scroll-animate" role="tablist" aria-label={t("eyebrow.about")}>
        {cards.map((card, idx) => (
          <button
            type="button"
            key={card.key}
          ...
          >
            <div className="pagination-svg">{card.svg}</div>
          </button>
        ))}
      </div>
